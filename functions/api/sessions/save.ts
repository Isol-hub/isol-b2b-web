...
export const onRequestPost: PagesFunction<Env> = async ({ request, env }) => {
  ...
  try {
    ...
    const { workspace_slug, target_lang, started_at, ended_at, transcript_lines, ai_formatted_text, ai_notes_text, highlights, speakers, wss_session_id } = body
    ...
    const sessionResult = await env.DB.prepare(
      `INSERT INTO sessions (workspace_slug, target_lang, started_at, ended_at, line_count, ai_formatted_text, ai_notes_text, wss_session_id)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`
    ).bind(
      workspace_slug,
      target_lang,
